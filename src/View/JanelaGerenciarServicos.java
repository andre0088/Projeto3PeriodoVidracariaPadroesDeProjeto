...
public class JanelaGerenciarServicos extends JanelaGerenciar {

	private OuvinteJanelaGerenciarServicos ouvinte = new OuvinteJanelaGerenciarServicos(this);

	public JanelaGerenciarServicos() {
		super("Lima Vidros - Gerenciamento de Serviços Pendentes");
		adicionarJLabel();
		addFiltro();
		adicionarJTable();
		atualizarTabela();
	...
	public void adicionarJLabel() {
		...
		listar = new JLabelPersonalizado("<html><div align=center>Listar Todos<br>os Serviços</div></html>", 10, 490,
				230, 60);
		listar.addMouseListener(ouvinte);
		add(editar);
		add(excluir);
		add(listar);
	}
	...
	public void atualizarTabela() {
		...
		while (it.hasNext()) {
			...
			if (ser.getStatus().equals("Concluído") == false) {
				String nome = "";
				...
				Object[] linha = new Object[] {
						ser.getId(), nome, ser.getEndereco(), ser.getPrecoCompleto(),ser.getQtdPago(),
						formato.format(ser.getDataPedido()), formato.format(ser.getDataEntrega()),
	...
	private class OuvinteJanelaGerenciarServicos implements MouseListener, KeyListener {

		private JanelaGerenciarServicos janela;

		public OuvinteJanelaGerenciarServicos(JanelaGerenciarServicos j) {
			janela = j;
		}
		...
		public void mouseReleased(MouseEvent e) {
			if (e.getSource() == listar) {
				new JanelaGerenciarServicosGerais().setLocationRelativeTo(janela);
				dispose();
			}
